 ...
 **CRITICAL:** Before powering on, you must access the GP2040-CE Web Config menu (`http://192.168.7.1`), navigate to **Pin Mapping**, and set `GPIO 4` and `GPIO 5` to `None`. If you do not clear the default arcade button mappings from these pins, the Pico will misinterpret the 1000Hz data stream as physical button mashing.
